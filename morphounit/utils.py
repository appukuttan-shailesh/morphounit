...
class NeuroM_MorphStats_pop(sciunit.Model):
    """A class to interact with morphology populations via the morphometrics-NeuroM's API (morph_stats)"""

    def __init__(self, model_name='NeuroM_MorphStats_pop', morph_path=None,
                 config_path_cells=None, morph_stats_file_cells=None,
                 config_path_pop=None, morph_stats_file_pop=None,
                 base_directory='.'):
        ...
        self.morph_path = morph_path
        self.config_path = config_path_cells
        self.config_path_pop = config_path_pop

        # Defining output directory
        self.morph_stats_output = os.path.join(base_directory, 'validation_results', 'neuroM_morph_softChecks',
                                               self.version, datetime.now().strftime("%Y%m%d-%H%M%S"))

        self.output_file = os.path.join(self.morph_stats_output, morph_stats_file_cells)
        self.output_file_pop = os.path.join(self.morph_stats_output, morph_stats_file_pop)
    ...
    def set_morph_feature_info(self):
        ...
        try:
            os.system('morph_stats -C {} -o {} {}'.format(self.config_path, self.output_file, self.morph_path))
            os.system('morph_stats -C {} -o {} {} --as-population'.format(self.config_path_pop, self.output_file_pop,
                                                                          self.morph_path))
        except IOError:
            print "Please specify the paths to a morphology directory and configuration files for morph_stats"
        ...
        for key0, dict0 in mod_prediction.items():  # Dict. with cell's morph_path-features dict. pairs for each cell
            ...
            mod_prediction.update({cell_ID: dict0})
        """
        with open(self.output_file_pop, 'r') as fp:
            mod_prediction = json.load(fp)
        for key0, dict0 in mod_prediction.items():  # Dict. with cell's morph_path-features dict. pairs for each cell
            cell_ID = (key0.split("/")[-1]).split(".")[0]
            del mod_prediction[key0]
        ...
        # Saving NeuroM's morph_stats output in a formatted json-file
        with open(self.output_file_pop, 'r+') as fp:
            pop_prediction = json.load(fp)
            fp.seek(0)
            json.dump(pop_prediction, fp, sort_keys=True, indent=4)
